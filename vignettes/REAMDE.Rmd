...
correlation <- call.pvcontainer(do_correlation, cylinders, correlation_method)
```
...
do_corr_summary <- function(param, value) {
    summary <- data.frame(corr = c(unlist(value[, 'corr_disp_hp']),
                                   unlist(value[, 'corr_disp_wt'])),
                          variable = c('HP', 'Weight'))
                          
    list(summary = summary)
}

corr_summary <- make_summary(call.pvcontainer(do_corr_summary, correlation))
```
...
We have covered the `call.pvcontainer` function which takes a function and one or more “parameter value container” objects and calls the function on each combination of the parameters.
