...
```{r}
correlation <- pv_call(do_correlation, cylinders)
```
...
correlation <- pv_call(do_correlation, cylinders, correlation_method)
```
...
corr_summary <- pv_call(do_corr_summary, correlation)
```
